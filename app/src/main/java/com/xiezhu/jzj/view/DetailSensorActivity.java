...
public class DetailSensorActivity extends DetailActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mStateIcon = findViewById(R.id.detailSensorImgStateIcon);
        mStateName = findViewById(R.id.detailSensorLblStateName);
        mStateValue = findViewById(R.id.detailSensorLblStateValue);
        mIcon = findViewById(R.id.detailSensorImgIcon);
        mPowerName = findViewById(R.id.detailSensorLblPowerName);
        mPowerValue = findViewById(R.id.detailSensorLblPowerValue);
        ...
        // 如果没有电池电源则不显示
        if (!mIsHasPowerSource) {
            RelativeLayout relativeLayoutPower = findViewById(R.id.detailSensorRLPower);
            relativeLayoutPower.setVisibility(View.GONE);
        }

        // 遥控按钮无状态显示栏
        if (mProductKey.equals(CTSL.PK_REMOTECONTRILBUTTON)) {
            RelativeLayout relativeLayoutState = findViewById(R.id.deteailSensorRLState);
            relativeLayoutState.setVisibility(View.GONE);
        }

        RelativeLayout mLayoutState2 = findViewById(R.id.deteailSensorRLState2);
        mLayoutState2.setVisibility(View.GONE);
        // 温湿度要使用状态2显示栏
        if (mProductKey.equals(CTSL.PK_TEMHUMSENSOR)) {
            mLayoutState2.setVisibility(View.VISIBLE);
            mStateIcon2 = findViewById(R.id.detailSensorImgStateIcon2);
            mStateIcon2.setImageResource(ImageProvider.genProductPropertyIcon(mProductKey, CTSL.THS_P_CurrentHumidity));
            mStateName2 = findViewById(R.id.detailSensorLblStateName2);
            mStateValue2 = findViewById(R.id.detailSensorLblStateValue2);
        }
        initStatusBar();
    }
