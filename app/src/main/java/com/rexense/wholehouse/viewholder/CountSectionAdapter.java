/*
 * Copyright (C) 2015 Tomás Ruiz-López.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.rexense.wholehouse.viewholder;

import android.content.Context;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.bumptech.glide.Glide;
import com.bumptech.glide.load.resource.drawable.DrawableTransitionOptions;
import com.rexense.wholehouse.R;
import com.rexense.wholehouse.model.EProduct;
import com.rexense.wholehouse.model.ProductTeam;
import com.rexense.wholehouse.presenter.ImageProvider;
import com.truizlop.sectionedrecyclerview.SectionedRecyclerViewAdapter;
import com.vise.log.ViseLog;

import java.util.List;

public class CountSectionAdapter extends SectionedRecyclerViewAdapter<CountHeaderViewHolder,
        CountItemViewHolder,
        CountFooterViewHolder> {

    private List<ProductTeam> mProductTeamList;
    protected Context mContext = null;

    public CountSectionAdapter(Context context, List<ProductTeam> productTeams) {
        mContext = context;
        mProductTeamList = productTeams;
    }

    @Override
    protected int getItemCountForSection(int section) {
        return mProductTeamList.get(section).getProductList().size();
    }

    @Override
    protected int getSectionCount() {
        return mProductTeamList.size();
    }

    @Override
    protected boolean hasFooterInSection(int section) {
        return false;
    }

    protected LayoutInflater getLayoutInflater() {
        return LayoutInflater.from(mContext);
    }

    @Override
    protected CountHeaderViewHolder onCreateSectionHeaderViewHolder(ViewGroup parent, int viewType) {
        View view = getLayoutInflater().inflate(R.layout.view_count_header, parent, false);
        return new CountHeaderViewHolder(view);
    }

    @Override
    protected CountFooterViewHolder onCreateSectionFooterViewHolder(ViewGroup parent, int viewType) {
        View view = getLayoutInflater().inflate(R.layout.view_count_footer, parent, false);
        return new CountFooterViewHolder(view);
    }

    @Override
    protected CountItemViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        View view = getLayoutInflater().inflate(R.layout.list_configproduct, parent, false);
        return new CountItemViewHolder(view);
    }

    @Override
    protected void onBindSectionHeaderViewHolder(CountHeaderViewHolder holder, int section) {
        //holder.render("Section " + (section + 1));
        holder.render(mProductTeamList.get(section).getTitle());
    }

    @Override
    protected void onBindSectionFooterViewHolder(CountFooterViewHolder holder, int section) {
        holder.render("Footer " + (section + 1));
    }

    protected int[] colors = new int[]{0xfff44336, 0xff2196f3, 0xff009688, 0xff8bc34a, 0xffff9800};

    @Override
    protected void onBindItemViewHolder(CountItemViewHolder holder, int section, int position) {
        ProductTeam team = mProductTeamList.get(section);
        EProduct.configListEntry product = team.getProductList().get(position);

        holder.nameTV.setText(product.name);
        Glide.with(mContext).load(product.image)
                .transition(new DrawableTransitionOptions().crossFade()).into(holder.imgIcon);
        holder.containerView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mListener.onItemClicked(section, position);
            }
        });
    }

    private CountSectionListener mListener;

    public void setListener(CountSectionListener listener) {
        this.mListener = listener;
    }

    public interface CountSectionListener {
        void onItemClicked(int section, int position);
    }
}
