...
public class USixSceneSwitchActivity2 extends DetailActivity {
    ...
    private static class MyResponseErrHandler extends Handler {
        private final WeakReference<USixSceneSwitchActivity2> ref;

        public MyResponseErrHandler(USixSceneSwitchActivity2 activity) {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            USixSceneSwitchActivity2 activity = ref.get();
    ...
    private static class DelSceneHandler extends Handler {
        ...
        public DelSceneHandler(USixSceneSwitchActivity2 activity) {
            ref = new WeakReference<>(activity);
        }
            ...
            if (msg.what == Constant.MSG_CALLBACK_EXTENDED_PROPERTY_GET) {
                if (msg.obj != null && !TextUtils.isEmpty((String) msg.obj)) {
                    ...
                    if (keyNo != null && keyNo.equals(activity.mPressedKey)) {
                        ...
                        activity.mSceneManager.deleteScene(autoSceneId,
...
//                            , sceneId.equals(activity.mFirstManualSceneId) ? activity.mFirstManualSceneName : activity.mSecondManualSceneName), Toast.LENGTH_LONG).show();
    ...
    // 响应错误处理器
    protected Handler mExtendedPropertyResponseErrorHandler = new Handler(msg -> {
        if (Constant.MSG_CALLBACK_APIRESPONSEERROR == msg.what) {
            EAPIChannel.responseErrorEntry responseErrorEntry = (EAPIChannel.responseErrorEntry) msg.obj;
            StringBuilder sb = new StringBuilder();
            sb.append(String.format("提交接口[%s]成功, 但是响应发生错误:", responseErrorEntry.path));
