package com.rexense.smart.widget;
 ...
 * Description: 圆形进度条控件
 */
public class ComCircularProgress extends View {
	private Paint mBackPaint, mProgPaint;   // 绘制画笔
	private RectF mRectF;       // 绘制区域
	private int[] mColorArray;  // 圆环渐变色
	private int mMaxProgress;	// 圆环最大进度
	private int mProgress;      // 圆环进度

	public ComCircularProgress(Context context) {
		this(context, null);
	}

	public ComCircularProgress(Context context, @Nullable AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public ComCircularProgress(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);
		@SuppressLint("Recycle")
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ComCircularProgress);
		...
		mBackPaint.setStrokeWidth(typedArray.getDimension(R.styleable.ComCircularProgress_backWidth, 5));
		mBackPaint.setColor(typedArray.getColor(R.styleable.ComCircularProgress_backColor, Color.LTGRAY));
		...
		mProgPaint.setStrokeWidth(typedArray.getDimension(R.styleable.ComCircularProgress_progWidth, 10));
		mProgPaint.setColor(typedArray.getColor(R.styleable.ComCircularProgress_progColor, Color.BLUE));

		// 初始化进度圆环渐变色
		int startColor = typedArray.getColor(R.styleable.ComCircularProgress_progStartColor, -1);
		int firstColor = typedArray.getColor(R.styleable.ComCircularProgress_progFirstColor, -1);
		if (startColor != -1 && firstColor != -1) mColorArray = new int[]{startColor, firstColor};
		else mColorArray = null;

		// 初始化最大进度
		mMaxProgress = typedArray.getInteger(R.styleable.ComCircularProgress_maxProgress, 100);

		// 初始化进度
		mProgress = typedArray.getInteger(R.styleable.ComCircularProgress_progress, 0);
		typedArray.recycle();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawArc(mRectF, 275, 360 * mProgress / mMaxProgress, false, mProgPaint);
	}
